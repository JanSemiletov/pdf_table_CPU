...
class PdfUtils(BaseUtil):
    ...
    def extract_pdf_to_text(file_name, text_save_dir, metric_save_dir, show_number=50, keep_page_number=True):
        ...
        if text_save_dir.endswith(".txt"):
            ...
            text_save_file = FileUtils.join_path(text_save_dir, f"{temp_file_name}.txt")

        metric_json_file = FileUtils.join_path(metric_save_dir, f"{temp_file_name}.json")
    ...
    def download_pdf_before_check(file_url, pdf_dir=None, file_name=None, ):
        ...
        if file_url.startswith("http"):
            ...
            pdf_file = FileUtils.join_path(pdf_dir, raw_file_name) if pdf_dir is not None else file_name
    ...
    def extract_pdf_image_page(file_name, output_dir,
        ...
        for index, image in enumerate(images):
            ...
            raw_image_name = FileUtils.join_path(output_dir, save_name)
            ...
            image_info = {
                ...
                "relative_dir": FileUtils.join_path(".", new_name),
                "bbox": image.bbox,
                "height": image.height,
                "width": image.width,
    ...
    def save_pdf_image(images: List[LTImage], output_dir, image_dir, read_cache=True):
        ...
        image_output_dir = FileUtils.join_path(output_dir, image_dir)
        ...
        for index, image in enumerate(images):
            ...
            save_name = image_writer.export_image(image)
            relative_dir = FileUtils.join_path(image_dir, save_name)

            image_info = {
                "key": PdfUtils.get_pdf_image_key(image),
                "name": image.name,
                "raw_name": save_name,
                "save_name": FileUtils.join_path(output_dir, relative_dir),
    ...
    def check_is_imaged_pdf_v2(file_name, pages: List = None, new_file=None, pdf_dir=None):
        ...
        if new_file is None:
            new_file = FileUtils.join_path(Constants.HTML_BASE_DIR, "check_imaged_pdf", TimeUtils.get_time(), FileUtils.get_raw_file_name(file_name))
