...
from pdftable.model.center_net.table_process import _tranpose_and_gather_feat
from pdftable.model.lore.configuration_lore import LoreConfig
from pdftable.model.lore.lineless_table_process import _get_4ps_feat, _get_wh_feat, _normalized_ps
...
def attention(q, k, v, d_k, mask=None, dropout=None):
    ...
    if mask is not None:
        ...
        scores = scores.masked_fill(mask2d == 0, -6.55e4)
    ...
    if scores.dtype == torch.float16:
        output = torch.matmul(scores.float(), v.float())
        output = output.half()
    else:
...
class LoreProcessModel(nn.Module):
    ...
    def __init__(self, config: LoreConfig, **kwargs):
        ...
        self.stacking_layers = config.stacking_layers
        self.tsfm_layers = config.tsfm_layers
        self.num_heads = 8
    ...
    def get_train_features(self, outputs, batch):
        # Training Mode
        ind = batch['hm_ind']
        ...
        elif self.config.wiz_4ps:
            cr_feat = _get_4ps_feat(batch['cc_match'], output)
            cr_feat = cr_feat.sum(axis=3)
            feat = ct_feat + cr_feat

        elif self.config.wiz_vanilla:
            feat = ct_feat
    ...
    def forward(self, outputs, batch=None, cc_match=None, dets=None):
        ...
        if batch is None:
            ...
            # Training Mode
            feat = self.get_train_features(outputs=outputs, batch=batch)
