...
class OcrSystemTask(object):
    ...
    def __call__(self, inputs, save_result=True, src_id=None, page=None, **kwargs) \
        ...
        # 绘制版面分析结果
        save_layout_path = FileUtils.join_path(self.output_dir, f"{raw_filename}_layout.jpg")
        if self.debug:
        ...
        if self.output_dir is not None and save_result and self.debug:
            ...
            save_file = FileUtils.join_path(self.output_dir, f"ocr_{raw_filename}_{run_time}.jpg")
            FileUtils.check_file_exists(save_file)

            # 绘制Text cell
            save_image_file = FileUtils.join_path(self.output_dir, f"{raw_filename}_text_cell.jpg")
