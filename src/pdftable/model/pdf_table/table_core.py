...
class Cell(object):
    ...
    def __init__(self, x1, y1, x2, y2, row_index=None,
                 col_index=None, inner_cells=None, logit_axis=None):
        self.x1 = x1
        self.y1 = y1
        self.x2 = x2
        self.y2 = y2
        ...
        self.height_ratio = 0
        self.text_item = None
        self.inner_cells: List = [] if inner_cells is None else inner_cells
        self.logit_axis: List = [] if logit_axis is None else logit_axis
        self.image_bbox: List = None
    ...
    def __repr__(self):
        ...
        label_list = self.get_span_label()
        label = '-'.join([str(item) for item in label_list])

        logit_label_list = self.get_pred_logit()
        logit_label = ','.join([str(item) for item in logit_label_list])

        pred_logit_axis = ','.join([str(item) for item in self.logit_axis]) if self.logit_axis is not None else ""

        return f"<Cell x1={x1} y1={y1} x2={x2} y2={y2} " \
               f"index=[{self.row_index},{self.col_index}] " \
               f"span=[{self.row_span},{self.col_span}] " \
               f"wight=[{self.width},{self.height}] " \
               f"wight_ratio=[{self.width_ratio:.2f},{self.height_ratio:.2f}] " \
               f"text=[{self.text}] " \
               f"line=[{self.left},{self.top},{self.right},{self.bottom}] " \
               f"label=[{label}] " \
               f"logit_label=[{logit_label}] " \
               f"pred_logit_axis=[{pred_logit_axis}] > "

    def get_pred_logit(self):
        logit_label_list = [self.col_index - 1, self.col_index - 2 + self.col_span,
                            self.row_index - 1, self.row_index - 2 + self.row_span, ]
        return logit_label_list

    def check_pred_logit(self):
        logit_label_list = self.get_pred_logit()
        flag = True
        if self.logit_axis is not None:
            for logit_label, logit_pred in zip(logit_label_list, self.logit_axis):
                if logit_label != logit_pred:
                    flag = False
                    break
        else:
            flag = False
    ...
    def get_span_label(self):
        label_list = [self.row_index, self.col_index, self.row_span, self.col_span, self.text]
        return label_list
    ...
    def points(self):
        point_list = [
            self.lb, self.lt, self.rb, self.rt
        ]
        return point_list

    def point_array(self, use_image_point=True):
        x1, y1, x2, y2 = self.to_bbox()
        if use_image_point and self.image_bbox is not None:
    ...
    @property
    def width(self):
        return round(abs(self.x2 - self.x1))

    @property
    def height(self):
        return round(abs(self.y2 - self.y1))
    ...
    @property
    def x1_round(self):
        return round(self.x1)

    @property
    def y1_round(self):
        return round(self.y1)

    @property
    def x2_round(self):
        return round(self.x2)

    @property
    def y2_round(self):
        return round(self.y2)

    def check_line_match(self, line, diff=2, match_radio=0.8):
        """
        判断 cell 是否和 line 重合
    ...
    def get_show_lines(self):
        lines = []
        if self.top:
            lines.append([self.lt, self.rt])
        if self.bottom:
            lines.append([self.lb, self.rb])
        if self.left:
            lines.append([self.lt, self.lb])
        if self.right:
            lines.append([self.rt, self.rb])
        return lines
...
class Table(object):
    ...
    def to_json(self, path, **kwargs):
        ...
        """
        kw = {"orient": "records", "force_ascii": False}
        kw.update(kwargs)
    ...
    def to_html(self, path, **kwargs):
        ...
        kw.update(kwargs)
        html_string = self.df.to_html(**kw)
        with open(path, "w", encoding="utf-8") as f:
            f.write(html_string)
...
class TableList(object):
    """Defines a list of Table objects. Each table can
